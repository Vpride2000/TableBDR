...
import { INVEST_REFERENCE_TABLES, CONTRACT_ROW_SEEDS, INVEST_PROGRAM_SEEDS, CONTRACT_ADDITIONAL_AGREEMENTS_SEEDS, } from '../config/config.js';
...
export async function ensureInvestReferenceTables(client) {
...
export async function ensureContractsTable(client) {
  ...
  `);
    ...
    for (const seed of CONTRACT_ROW_SEEDS) {
        await client.query(`INSERT INTO "GN_contracts" (
       ...
       ) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [seed.contractorId, seed.dogovorId, seed.sedLaunchDate, seed.asezLoadDate, seed.state, seed.statusUpdatedAt, seed.approvalStatus || 'действующий']);
    }
}
// Обеспечивает наличие таблицы инвестпрограммы и начальное заполнение.
export async function ensureInvestProgramTable(client) {
    await client.query(`
  ...
  `);
    ...
    for (const seed of INVEST_PROGRAM_SEEDS) {
        await client.query(`INSERT INTO "GN_invest_program" (
         "GN_invest_pf_npf",
         "GN_invest_name",
         "GN_invest_quantity",
         "GN_invest_okdp_fk",
...
export async function ensureContractAdditionalAgreementsTable(client) {
    await client.query(`
  ...
  `);
    ...
    for (const seed of CONTRACT_ADDITIONAL_AGREEMENTS_SEEDS) {
        await client.query(`INSERT INTO "GN_contract_additional_agreements" (
         "GN_contract_id_FK",
...
export async function ensureLimitCalculationTable(client) {
    await client.query(`
...
export async function ensureForecastMonthlyTable(client) {
    await client.query(`
