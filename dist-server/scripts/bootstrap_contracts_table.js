...
async function main() {
    ...
    `);
        const countResult = await client.query(`SELECT COUNT(*)::text AS count FROM "GN_contracts"`);
        if (Number(countResult.rows[0]?.count ?? '0') === 0) {
           ...
           ) VALUES ($1, $2, $3, $4, $5, $6)`, [seed.contractorId, seed.dogovorId, seed.sedLaunchDate, seed.asezLoadDate, seed.state, seed.statusUpdatedAt]);
            }
        }
        const verify = await client.query(`SELECT COUNT(*)::text AS count FROM "GN_contracts"`);
...
main();
//# sourceMappingURL=bootstrap_contracts_table.js.map